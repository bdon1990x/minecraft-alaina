...
- training model to build structures in Minecraft using training data created by a generative adversarial network
- initially we thought about structures such as buildings, but have focused on simpler structures such as chairs, tables, etc etc
...
- use a dataset of 3D objects (specifically chairs, maybe add more later)
- with Python scripts, voxelize the models from the above dataset
- above scripts output a 3D matrix that is binary encoded
- 0 = empty, 1 = cube; matrix dim = 30x30x30
- use above matrix as input to GANs to create training data (input is flattened matrix)
- with GANs, we train a discriminatory and generator model
